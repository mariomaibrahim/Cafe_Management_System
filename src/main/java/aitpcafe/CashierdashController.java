...
public class CashierdashController implements Initializable {
    ...
    @FXML private Label cashierName, cashierPhone, cashierEmail, cashierSalary;
    @FXML private ProgressBar performanceBar;
    @FXML private Label performanceLabel, performanceStatus;
    ...
    @FXML private TableColumn<Customer, Integer> col_custId, col_custOrders;
    @FXML private TableColumn<Customer, String> col_custName, col_custPhone, col_custLastVisit;
    @FXML private TableColumn<Customer, Double> col_custTotal;
    ...
    private void setupCustomersTable() {
        ...
        col_custName.setCellValueFactory(new PropertyValueFactory<>("name"));
        col_custPhone.setCellValueFactory(new PropertyValueFactory<>("phone"));
        col_custOrders.setCellValueFactory(new PropertyValueFactory<>("orderCount"));
        col_custTotal.setCellValueFactory(new PropertyValueFactory<>("totalSpent"));
        col_custLastVisit.setCellValueFactory(new PropertyValueFactory<>("lastVisit"));
        
        // Format total column
        col_custTotal.setCellFactory(column -> new TableCell<Customer, Double>() {
            @Override
            protected void updateItem(Double item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
    ...
    private void loadCustomers() {
        ...
        try {
            conn = DatabaseConfig.getConnection();
            String query = "SELECT cr.customer_id, cr.customer_name, " +
                          "COUNT(*) as order_count, " +
                          "SUM(cr.final_amount) as total_spent, " +
                          "MAX(cr.date) as last_visit, " +
                          "c.phone " +
                          "FROM customer_receipt cr " +
                          "LEFT JOIN customer c ON cr.customer_id = c.customer_id " +
                          "WHERE cr.em_username = ? " +
                          "GROUP BY cr.customer_id, cr.customer_name, c.phone " +
            ...
            while (rs.next()) {
                Customer customer = new Customer(
                    ...
                    rs.getString("last_visit") != null ? rs.getString("last_visit") : "N/A"
                );
    ...
    public static class Customer {
        ...
        private String name, phone, lastVisit;
        private Integer orderCount;
        private Double totalSpent;

        public Customer(Integer customerId, String name, String phone, Integer orderCount, Double totalSpent, String lastVisit) {
