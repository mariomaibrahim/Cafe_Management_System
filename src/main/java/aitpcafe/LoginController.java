...
public class LoginController implements Initializable {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (verifyPassword(password, storedPassword)) {
                    // Initialize UserSession with all user data
    ...
    private boolean verifyPassword(String inputPassword, String storedPassword) {
        ...
        // Check if it's a hashed password (SHA-256 produces 64 hex characters)
        if (storedPassword.length() == 64) {
            String hashedInput = hashPassword(inputPassword);
            return hashedInput != null && hashedInput.equals(storedPassword);
        } else {
            // Legacy plain text password support (should be migrated)
